...
namespace PointCloudViewer
{
	...
	ShaderTable::ShaderTable(uint64_t size, const void* shaderIdentifier):
		m_size(math::align<uint64_t>(D3D12_SHADER_IDENTIFIER_SIZE_IN_BYTES + size, D3D12_RAYTRACING_SHADER_RECORD_BYTE_ALIGNMENT))
	{
	...
	{
		m_raytracingShader = std::make_unique<Shader>(
			args.shaderPath,
		...
		m_globalInputContainer.InitContainer(m_raytracingShader->GetInputMap(), D3D12_ROOT_SIGNATURE_FLAG_NONE);
		...
		D3D12_DXIL_LIBRARY_DESC libraryDesc{
			...
			.NumExports = static_cast<uint32_t>(m_raytracingShader->GetLocalInputMaps().size()),
		...
		auto CreateShaderTable = [this](ShaderTableType type, void* shaderIdentifier)
		{
			...
			for (const auto& pair : m_raytracingShader->GetLocalInputMaps().at(type))
			{
